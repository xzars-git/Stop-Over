    ...
    public class NarrativeCondition : MonoBehaviour
    {
        ...
        public bool IsMet(GameObject triggerer)
        {
            ...
            if (type == NarrativeConditionType.HoldItem)
            {
                ...
                {
                    IndieMarc.Platformer.CharacterHoldItem character = targ.GetComponent<IndieMarc.Platformer.CharacterHoldItem>();
                    IndieMarc.Platformer.CarryItem item = character.GetHeldItem();
                ...
                {
                    IndieMarc.TopDown.CharacterHoldItem character = targ.GetComponent<IndieMarc.TopDown.CharacterHoldItem>();
                    IndieMarc.TopDown.CarryItem item = character.GetHeldItem();
                    condition_met = (item != null && (item.item_type == value_string || value_string == ""));
                }
                if (oper2 == NarrativeConditionOperator2.IsFalse)
                {
            ...
            if (type == NarrativeConditionType.DialogueTriggered)
            {
                ...
                if (targ && targ.GetComponent<DialogueEvent>())
                {
                    ...
                    condition_met = CompareInt(cinema.GetTriggerCount(), value_int);
                }
            }
