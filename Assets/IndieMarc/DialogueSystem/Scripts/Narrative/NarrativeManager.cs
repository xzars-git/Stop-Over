...
namespace IndieMarc.DialogueSystem
{
    public class NarrativeManager : MonoBehaviour
    {
        ...
        [HideInInspector] public UnityAction<DialogueEvent> OnCinematicStart;
        [HideInInspector] public UnityAction<DialogueEvent> OnCinematicEnd;
        ...
        public void StartEvent(DialogueEvent dialogue_event)
        {
            ...
            if (current_event != dialogue_event)
            {
                ...
                {
                    IndieMarc.Platformer.FollowCamera pcam = IndieMarc.Platformer.FollowCamera.Get();
                    IndieMarc.TopDown.FollowCamera tdcam = IndieMarc.TopDown.FollowCamera.Get();
                    //TheCamera.GetCurrent().LockCameraOn(current_cinematic.camera_lock_on);
                }

                if (OnCinematicStart != null)
                {
                    OnCinematicStart.Invoke(dialogue_event);
                }
        ...
        public void StopEvent(bool interupted = false)
        {
            ...
            if (trigger != null && trigger.camera_lock_on != null)
            {
                IndieMarc.Platformer.FollowCamera pcam = IndieMarc.Platformer.FollowCamera.Get();
                IndieMarc.TopDown.FollowCamera tdcam = IndieMarc.TopDown.FollowCamera.Get();
                //TheCamera.GetCurrent().UnlockCamera();
            }
        ...
        public void StartDialogue(DialogueMessage dialogue)
        {
            ...
            if (current_actor != null)
            {
                ...
                side = current_message.side != 0 ? current_message.side : 1;
                flipped = side >= 1;
        ...
        public GameObject GetNearestPlayer(Vector3 pos, float radius, bool alive_only=true)
        {
            ...
            IndieMarc.Platformer.PlayerCharacter pcharacter = IndieMarc.Platformer.PlayerCharacter.GetNearest(pos, radius, alive_only);
            if (pcharacter != null)
                player = pcharacter.gameObject;

            IndieMarc.TopDown.PlayerCharacter tdcharacter = IndieMarc.TopDown.PlayerCharacter.GetNearest(pos, radius, alive_only);
            if (tdcharacter != null)
                player = tdcharacter.gameObject;
