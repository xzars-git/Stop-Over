...
namespace IndieMarc.DialogueSystem
{
    ...
    public class NarrativeEvent : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                if (trigger_target.GetComponent<IndieMarc.Platformer.CarryItem>())
                {
                    trigger_target.GetComponent<IndieMarc.Platformer.CarryItem>().OnTake += OnTriggerEvent;
                }
                else if (trigger_target.GetComponent<IndieMarc.TopDown.CarryItem>())
                {
                    trigger_target.GetComponent<IndieMarc.TopDown.CarryItem>().OnTake += OnTriggerEvent;
                }
            }

            if (trigger_type == NarrativeEventType.UseLever)
            {
                if (trigger_target.GetComponent<IndieMarc.Platformer.Lever>())
                {
                    trigger_target.GetComponent<IndieMarc.Platformer.Lever>().OnTriggerLever += OnTriggerEvent;
                }
                if (trigger_target.GetComponent<IndieMarc.TopDown.Lever>())
                {
                    trigger_target.GetComponent<IndieMarc.TopDown.Lever>().OnTriggerLever += OnTriggerEvent;
                }
            }

            if (trigger_type == NarrativeEventType.CharacterDead)
            {
                if (trigger_target.GetComponent<IndieMarc.Platformer.PlayerCharacter>())
                {
                    trigger_target.GetComponent<IndieMarc.Platformer.PlayerCharacter>().onDeath += OnTriggerEvent;
                }
                if (trigger_target.GetComponent<IndieMarc.TopDown.PlayerCharacter>())
                {
                    trigger_target.GetComponent<IndieMarc.TopDown.PlayerCharacter>().onDeath += OnTriggerEvent;
                }
            }
        }
