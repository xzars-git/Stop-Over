...
namespace IndieMarc.DialogueSystem
{
    ...
    public class NarrativeEffect : MonoBehaviour
    {
        ...
        public void Trigger(GameObject triggerer)
        {
            ...
            {
                GameObject targ = target.GetTargetObject(triggerer);
                if (targ && targ.GetComponent<IndieMarc.Platformer.CharacterHoldItem>())
                {
                    IndieMarc.Platformer.CarryItem item = targ.GetComponent<IndieMarc.Platformer.CharacterHoldItem>().GetHeldItem();
                    if (item)
                        Destroy(item.gameObject);
                }
                if (targ && targ.GetComponent<IndieMarc.TopDown.CharacterHoldItem>())
                {
                    IndieMarc.TopDown.CarryItem item = targ.GetComponent<IndieMarc.TopDown.CharacterHoldItem>().GetHeldItem();
                    if (item)
                        Destroy(item.gameObject);
                }
            }
            ...
            if (type == NarrativeEffectType.LockCamera)
            {
                ...
                if (tdcam != null)
                    tdcam.LockCameraOn(value_object);
            }
            ...
            {
                foreach (IndieMarc.Platformer.PlayerCharacter character in IndieMarc.Platformer.PlayerCharacter.GetAll())
                    character.DisableControls();
                foreach (IndieMarc.TopDown.PlayerCharacter character in IndieMarc.TopDown.PlayerCharacter.GetAll())
                    character.DisableControls();
            }
